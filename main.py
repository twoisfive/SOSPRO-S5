...
def home():
    ...
    return render_template("main.html", main_section_files=main_section_files)
...
def about_us():
    ...
    return render_template("about_us.html", about_us_section_files=section_files)

@app.route("/event")
def event():
    section_folder = "templates/sections/event_sections"
    section_files = sorted(f for f in os.listdir(section_folder) if f.endswith(".html"))
    return render_template("event.html", event_section_files=section_files)

@app.route("/pacil")
def pacil():
    section_folder = "templates/sections/pacil_sections"
    section_files = sorted(f for f in os.listdir(section_folder) if f.endswith(".html"))
    return render_template("pacil.html", pacil_section_files=section_files)
